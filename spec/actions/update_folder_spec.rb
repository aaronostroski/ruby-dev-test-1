...
RSpec.describe UpdateFolder do
  ...
  describe 'Happy path' do
    it 'User updates folder with no parent folder' do
      ...
      expect(action.folder.parent).to be_nil
    ...
    it 'User updates folder with parent folder' do
      ...
      parent = FactoryBot.create(:folder)
      action = described_class.new(folder_id: folder.id, name: 'New Folder', description: 'Description', parent_id: parent.id)
      ...
      expect(action.folder.name).to eq 'New Folder'
      expect(action.folder.description).to eq 'Description'
      expect(action.folder.parent).to eq parent
      expect(action.folder.archives).to be_empty
    end
  end
  ...
  describe 'Validations' do
    it 'If parent folder is not found, it should return an error' do
      action = described_class.new(folder_id: folder.id, name: 'New Folder', description: 'Description', parent_id: 9999)
